...
from nnunet.inference.segmentation_export import save_segmentation_nifti_from_softmax, save_segmentation_nifti_from_softmaxs
...
def merge_files2(args):
    files, properties_files, out_file, only_keep_largest_connected_component, min_region_size_per_class, override = args
    if override or not isfile(out_file):
        softmax = [np.load(f)['softmax'] for f in files]
        props = [load_pickle(f) for f in properties_files]
        save_segmentation_nifti_from_softmaxs(softmax, out_file, props, 1, None, None, None)
...
def merge(folders, output_folder, threads, override=True):
    ...
    only_keep_largest_connected_component, min_region_size_per_class = None, None
    p = Pool(threads)
    p.map(merge_files2, zip(files, property_files, out_files, [only_keep_largest_connected_component] * len(out_files),
