...
import numpy as np
from uuunet.evaluation.add_mean_dice_to_json import foreground_mean
from subprocess import call
import SimpleITK as sitk
from uuunet.run.default_configuration import get_output_folder
        ...
        if len(json_files_task) > 0:
            ...
            for i in range(len(json_files_task) - 1):
                for j in range(i+1, len(json_files_task)):
                    ...
                    p = call(["python", join(uuunet.__path__[0], "evaluation/model_selection/ensemble.py"), network1_folder, network2_folder, output_folder, task_name])
