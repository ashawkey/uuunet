...
def restore_model(pkl_file, checkpoint=None, train=False):
    ...
    uuunet.trainig.network_training for the file that contains the trainer and instantiate it with the arguments saved in the pkl file. If checkpoint
    ...
    init = info['init']
    name = info['name']
    search_in = join(uuunet.__path__[0], "training", "network_training")
    tr = recursive_find_trainer([search_in], name, current_module="uuunet.training.network_training")

    if tr is None:
        raise RuntimeError("Could not find the model trainer specified in checkpoint in uuunet.trainig.network_training. If it "
                           "is not located there, please move it or change the code of restore_model. Your model "
                           "trainer can be located in any directory within uuunet.trainig.network_training (search is recursive)."
...
def load_model_and_checkpoint_files(folder, folds=None):
    ...
    ### HAW! Manually setting the network bothers!
    from uuunet.MAGIC import XNet
    print("#######################################")
    print("###  training/model_restore.py 130  ###")
    print("###  XNet                           ###")
    print("### best                            ###")
    print("#######################################")
    trainer.network = XNet().cuda()

    ### HAW, it uses the best model.
    #all_best_model_files = [join(i, "model_latest.model") for i in folds]
