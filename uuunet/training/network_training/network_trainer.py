...
class NetworkTrainer(object):
    ...
    ##########################################
    # God bless you! there are 4 functions for loading a fucking checkpoint?
    ...
    #####################################################
    # The training function! Maybe the force be with you!

    def run_training(self):
        print("[HAW] this shall never be called, you run into bugs.")
    ...
    def update_eval_criterion_MA(self):
        ...
        # seems the all_val_eval_metrics is not used at all ???
    ...
    #####################################################
    # this is the train_step / eval_step function !!!!!!!
    # add modality.
    # add tensorboardx visualization.

    def run_iteration(self, data_generator, modality, do_backprop=True, run_online_evaluation=False):
        ...
        if threading.current_thread() is threading.main_thread():
            print("!!!input", data.shape, target.shape) # haw
            matplotlib.use('TkAgg')
            if len(data.shape) == 5:
                view_batch(data[0][0], target[0][0])
            else:
                plot_images(data[0][0], target[0][0])
        """
        ...
        # WOW! the FORWARD!
        output = self.network(data, modality)

        ### View output
        """
        output_ = output.detach().cpu().numpy().argmax(1)
        target_ = target.detach().cpu().numpy()
        if threading.current_thread() is threading.main_thread():
            print("!!!output", output_.shape, target_.shape) # haw
            matplotlib.use('TkAgg')
            if len(data.shape) == 5:
                view_batch(output_[0], target_[0][0])
            else:
                plot_images(output_[0], target_[0][0])
        """
